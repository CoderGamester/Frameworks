...
- **Both halves write the same `persistentDataPath/TestResults.xml`, so snapshot the Editor result before running batchmode.** `Tools/test-all.sh editor-save` does it; skip it and batchmode silently overwrites the Editor artifact, after which `compare` reads the batch run twice and prints `BOTH ENVIRONMENTS GREEN` having compared one environment with itself. Recorded instance: a compare that "passed" while the Editor file it claimed to read had a `start-time` seven minutes older than the run it was attributed to. `compare` now fails when the two files share a `start-time`, but the ordering is still the real fix — the guard only catches the collision, it cannot recover the lost artifact.
