...
- Run `python3 Tools/lint-agent-guides.py --self-test` after changing the linter, then run `python3 Tools/lint-agent-guides.py` after any guide, wrapper, skill-layout, or `.agents/skills/*/scripts/*.py` change. Editing `unity-package-release/scripts/release.py` additionally requires `python3 .agents/skills/unity-package-release/scripts/check_skill_docs.py` and `python3 -m unittest discover -s .agents/skills/unity-package-release/scripts -p 'test_*.py'` to pass.
