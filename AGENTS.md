...
- **NEVER documented — no exceptions.** Anything `private`: private methods, private fields, private properties, private events, and private or nested-private classes, structs and enums. Also **every constructor**, whatever its access, and generated code. If a symbol is `private` it gets no `///` block at all — when it needs explaining, put the rationale in a `//` comment per **Code comments** above. This is the most frequently broken rule in this repo; a doc comment on a private member is a defect, not a bonus.
